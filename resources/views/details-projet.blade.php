...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        [data-theme="dark"] .logo-image { filter: none; }

        .logo-text { display: flex; flex-direction: column; }
        .logo-main { font-size: 1.2rem; font-weight: 700; color: white; letter-spacing: -0.02em; }
        ...
        .user-info {
            ...
            background: rgba(255,255,255,0.08); padding: 0.35rem 1rem;
        ...
        .meta-pill {
            ...
            color: rgba(255,255,255,0.7); padding: 0.35rem 1rem; border-radius: 9999px;
        ...
        .theme-btn:hover { background: #3b82f6; color: white; border-color: #3b82f6; }

        .nav-container { max-width: 1600px; margin: 0 auto; padding: 0 2rem; }

        .nav-wrap {
            display: flex; gap: 0.25rem; background: rgba(255,255,255,0.1);
            border: 1px solid rgba(255,255,255,0.08); padding: 0.4rem;
            border-radius: 9999px; margin-top: 0.75rem; width: fit-content;
        }

        .nav-item {
            padding: 0.45rem 1.25rem; border-radius: 9999px; font-size: 0.8rem;
            font-weight: 500; color: rgba(255,255,255,0.6); text-decoration: none;
        ...
        .detail-card:hover { box-shadow: var(--shadow-md); border-color: var(--border-dark); }
        ...
        .info-item {
            background: var(--surface-hover); padding: 1rem 1.25rem;
            border-radius: var(--radius-md); border-left: 3px solid var(--accent);
        }
        .info-label { font-size: 0.65rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-muted); margin-bottom: 0.4rem; }
        .info-value { font-weight: 600; font-size: 1rem; color: var(--text-primary); }
        .info-sub { font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem; }

        .status-badge { padding: 0.25rem 1rem; border-radius: 100px; font-weight: 600; font-size: 0.7rem; display: inline-block; letter-spacing: 0.02em; }
        .status-badge.finalized { background: var(--success-light); color: var(--success); }
        .status-badge.delayed { background: var(--danger-light); color: var(--danger); }
        .status-badge.in-progress { background: var(--warning-light); color: var(--warning); }
        .status-badge.planned { background: var(--info-light); color: var(--info); }

        .norme-pill {
            background: var(--surface-hover); border: 1px solid var(--border);
            color: var(--text-secondary); padding: 0.2rem 0.9rem; border-radius: 100px;
            font-size: 0.75rem; font-weight: 500; display: inline-block; margin: 0.2rem; transition: all 0.2s;
        }
        .norme-pill:hover { border-color: var(--accent); background: var(--accent-soft); color: var(--accent); }

        .kpi-box { background: var(--surface-hover); border-radius: var(--radius-md); padding: 1.2rem 1rem; text-align: center; transition: transform 0.2s; }
        .kpi-box:hover { transform: translateY(-2px); }
        .kpi-label { font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem; font-weight: 600; }
        .kpi-value { font-size: 2rem; font-weight: 700; color: var(--text-primary); line-height: 1; }

        .progress-container { background: var(--border); height: 6px; border-radius: 100px; overflow: hidden; margin-top: 0.8rem; }
        .progress-fill { height: 100%; border-radius: 100px; background: linear-gradient(90deg, var(--accent), var(--accent-light)); transition: width 0.3s ease; }

        .table-pro { width: 100%; border-collapse: collapse; font-size: 0.82rem; }
        .table-pro thead th { background: var(--surface-hover); color: var(--text-muted); padding: 0.7rem 0.75rem; font-weight: 600; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.04em; border-bottom: 2px solid var(--border); white-space: nowrap; }
        .table-pro tbody td { padding: 0.7rem 0.75rem; border-bottom: 1px solid var(--border); color: var(--text-secondary); vertical-align: middle; }
        .table-pro tbody tr:last-child td { border-bottom: none; }
        .table-pro tbody tr:hover td { background: var(--surface-hover); }

        .print-col { width: 40px; text-align: center; }
        .btn-print-chapter { background: transparent; border: 1px solid var(--border); color: var(--text-muted); width: 30px; height: 30px; border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; font-size: 0.9rem; }
        .btn-print-chapter:hover { background: var(--accent); border-color: var(--accent); color: white; }

        .phase-badge { font-size: 0.7rem; font-weight: 600; padding: 0.25rem 1rem; border-radius: 100px; display: inline-block; }
        .phase-completed { background: var(--success-light); color: var(--success); }
        .phase-in-progress { background: var(--warning-light); color: var(--warning); }
        .phase-started { background: var(--info-light); color: var(--info); }
        .phase-not-started { background: var(--surface-hover); color: var(--text-muted); }

        .livrables-toggle { border: 1px solid var(--border); background: var(--surface-hover); color: var(--text-secondary); padding: 0.2rem 0.8rem; font-size: 0.7rem; border-radius: var(--radius-sm); font-weight: 500; transition: all 0.2s; }
        ...
        .livrable-status { display: inline-block; border-radius: 100px; padding: 0.1rem 0.7rem; font-size: 0.65rem; font-weight: 600; }
        .livrable-status.completed { background: var(--success-light); color: var(--success); }
        .livrable-status.in-progress { background: var(--warning-light); color: var(--warning); }
        .livrable-status.pending { background: var(--surface-hover); color: var(--text-muted); }

        .btn-preuve-voir { width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--accent); background: var(--accent-soft); color: var(--accent); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; font-size: 0.8rem; }
        .btn-preuve-voir:hover { background: var(--accent); color: white; transform: scale(1.1); }

        .preuve-count-badge { font-size: 0.6rem; font-weight: 700; padding: 0.1rem 0.4rem; border-radius: 100px; background: var(--accent); color: white; min-width: 18px; text-align: center; }
        .preuve-count-badge.zero { background: var(--surface-hover); color: var(--text-muted); }

        .preuve-viewer-modal { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); display: none; align-items: center; justify-content: center; z-index: 11000; }
        .preuve-viewer-modal.active { display: flex; }

        .preuve-viewer-content { background: var(--surface); border-radius: var(--radius-lg); width: 90%; max-width: 800px; max-height: 90vh; overflow: hidden; display: flex; flex-direction: column; }
        .preuve-viewer-header { padding: 1rem 1.5rem; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
        .preuve-viewer-header h5 { font-size: 0.9rem; font-weight: 600; color: var(--text-primary); margin: 0; }
        .btn-close-viewer { background: transparent; border: none; color: var(--text-muted); font-size: 1.2rem; cursor: pointer; }
        .btn-close-viewer:hover { color: var(--danger); }
        .preuve-viewer-body { flex: 1; overflow: auto; padding: 1.5rem; }
        .preuve-viewer-body img { max-width: 100%; max-height: 70vh; display: block; margin: 0 auto; }
        .preuve-viewer-body .pdf-embed { width: 100%; height: 70vh; border: none; }
        .preuve-viewer-body .file-preview { text-align: center; padding: 2rem; }

        .btn-download { background: var(--accent); color: white; border: none; border-radius: 8px; padding: 0.6rem 1.2rem; font-size: 0.8rem; cursor: pointer; display: inline-flex; align-items: center; gap: 0.4rem; }
        .btn-download:hover { background: var(--accent-light); }

        .preuve-list { display: flex; flex-direction: column; gap: 0.5rem; margin-top: 0.5rem; }
        .preuve-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.5rem 0.75rem; background: var(--surface-hover); border: 1px solid var(--border); border-radius: var(--radius-sm); cursor: pointer; }
        .preuve-item:hover { border-color: var(--accent); }
        .preuve-thumb { width: 40px; height: 40px; border-radius: 4px; object-fit: cover; border: 1px solid var(--border); }
        .preuve-thumb-icon { width: 40px; height: 40px; border-radius: 4px; background: var(--accent-soft); display: flex; align-items: center; justify-content: center; color: var(--accent); font-size: 1.2rem; }
        .preuve-info { flex: 1; }
        .preuve-info h6 { font-size: 0.75rem; font-weight: 600; color: var(--text-primary); margin: 0 0 0.2rem; }
        .preuve-info p { font-size: 0.65rem; color: var(--text-muted); margin: 0; }

        .intervention-section { margin-top: 2rem; }
        .intervention-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; }
        .intervention-header h4 { font-size: 1rem; font-weight: 600; color: var(--text-primary); display: flex; align-items: center; gap: 0.5rem; }
        .intervention-header h4 i { color: var(--accent); }
        .intervention-count { background: var(--accent-soft); color: var(--accent); padding: 0.25rem 1rem; border-radius: 100px; font-size: 0.8rem; font-weight: 600; }

        .intervention-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.25rem; margin-top: 1rem; }
        .intervention-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg); overflow: hidden; transition: all 0.2s; box-shadow: var(--shadow-sm); }
        .intervention-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-md); border-color: var(--accent); }
        .intervention-thumb { height: 150px; background: linear-gradient(135deg, var(--surface-hover), var(--surface)); display: flex; align-items: center; justify-content: center; overflow: hidden; border-bottom: 1px solid var(--border); }
        .intervention-thumb img { width: 100%; height: 100%; object-fit: cover; }
        .intervention-thumb i { font-size: 3.5rem; color: var(--accent); }
        .intervention-body { padding: 1rem; }
        .intervention-body h5 { font-size: 0.9rem; font-weight: 600; color: var(--text-primary); margin-bottom: 0.25rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .intervention-meta { font-size: 0.7rem; color: var(--text-muted); display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem; }
        .intervention-meta span { display: flex; align-items: center; gap: 0.25rem; }
        .intervention-meta i { font-size: 0.65rem; color: var(--accent); }
        .intervention-actions { display: flex; gap: 0.5rem; justify-content: flex-end; border-top: 1px solid var(--border); padding-top: 0.75rem; }
        .intervention-btn { width: 32px; height: 32px; border-radius: 8px; border: 1px solid var(--border); background: var(--surface-hover); color: var(--text-muted); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; }
        .intervention-btn:hover { background: var(--accent); color: white; border-color: var(--accent); }
        .intervention-empty { grid-column: 1 / -1; text-align: center; padding: 3rem; background: var(--surface-hover); border: 2px dashed var(--border); border-radius: var(--radius-lg); color: var(--text-muted); }
        .intervention-empty i { font-size: 3rem; color: var(--border); margin-bottom: 0.5rem; }
        .intervention-empty p { font-size: 0.9rem; margin-bottom: 0.25rem; }
        .intervention-empty small { font-size: 0.75rem; color: var(--text-muted); }

        .print-modal { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: none; align-items: center; justify-content: center; z-index: 10000; backdrop-filter: blur(4px); }
        .print-modal.active { display: flex; }
        .print-modal-content { background: var(--surface); border-radius: var(--radius-lg); width: 90%; max-width: 900px; max-height: 85vh; overflow-y: auto; padding: 2rem; position: relative; box-shadow: 0 20px 40px rgba(0,0,0,0.2); border: 1px solid var(--border); }
        .modal-close { position: absolute; top: 1.25rem; right: 1.25rem; background: var(--surface-hover); border: 1px solid var(--border); width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; color: var(--text-muted); }
        .modal-close:hover { background: var(--danger); border-color: var(--danger); color: white; }
        .btn-print-action { background: var(--accent); color: white; border: none; padding: 0.7rem 2rem; border-radius: 100px; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.2s; }
        .btn-print-action:hover { background: var(--accent-light); transform: translateY(-2px); }

        .print-document { padding: 0.5rem; }
        .print-header { border-bottom: 2px solid var(--accent); margin-bottom: 2rem; padding-bottom: 1rem; display: flex; justify-content: space-between; align-items: flex-end; }
        .print-title { font-size: 1.5rem; font-weight: 700; color: var(--accent); margin-bottom: 0.25rem; }
        .print-subtitle { font-size: 0.8rem; color: var(--text-muted); }
        .print-date { font-size: 0.8rem; color: var(--text-muted); background: var(--surface-hover); padding: 0.4rem 1rem; border-radius: 100px; }
        .print-section { margin-bottom: 2rem; }
        .print-section-title { font-size: 1rem; font-weight: 600; color: var(--text-primary); margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border); }
        .print-info-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; background: var(--surface-hover); padding: 1.25rem; border-radius: var(--radius-md); }
        .print-info-row { display: flex; border-bottom: 1px dashed var(--border); padding: 0.6rem 0; }
        .print-info-label { width: 130px; font-weight: 600; color: var(--text-muted); font-size: 0.8rem; }
        .print-info-value { flex: 1; color: var(--text-primary); font-weight: 500; font-size: 0.9rem; }
        .print-table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
        .print-table th { background: var(--surface-hover); color: var(--text-muted); font-size: 0.7rem; font-weight: 600; text-transform: uppercase; padding: 0.6rem; border: 1px solid var(--border); }
        .print-table td { padding: 0.6rem; border: 1px solid var(--border); color: var(--text-secondary); font-size: 0.8rem; }
        .print-footer { margin-top: 2rem; padding-top: 1rem; border-top: 2px solid var(--accent); text-align: center; color: var(--text-muted); font-size: 0.75rem; }

        .auto-badge { display: inline-block; font-size: 0.6rem; background: var(--accent-soft); color: var(--accent); padding: 0.1rem 0.4rem; border-radius: 100px; margin-left: 0.3rem; font-weight: 500; }

        @media (max-width: 768px) {
            .info-grid { grid-template-columns: 1fr; }
            .nav-wrap { flex-wrap: wrap; border-radius: var(--radius-md); width: 100%; }
        ...
        @media print {
            ...
            .intervention-btn, .intervention-section { display: none !important; }
...
<div class="print-modal" id="printModal">
    <div class="print-modal-content" id="printContent">
        <div class="modal-close" onclick="closePrintModal()"><i class="bi bi-x"></i></div>
...
<div class="preuve-viewer-modal" id="preuveViewerModal">
    <div class="preuve-viewer-content">
        ...
        <div class="preuve-viewer-body" id="preuveViewerBody"></div>
    </div>
</div>
...
<div class="container py-4">
    ...
    <div class="detail-card">
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="info-value"><span class="status-badge {{ $statusClass }}">{{ $projet->statut }}</span></div>
            </div>
            <div class="info-item">
    ...
    <div class="detail-card">
        ...
        <div class="row g-3">
            <div class="col-md-3 col-6">
                <div class="kpi-box"><div class="kpi-label">Jours prévus</div><div class="kpi-value">{{ $projet->jours_prevus }}</div></div>
            </div>
            <div class="col-md-3 col-6">
                <div class="kpi-box">
            ...
            <div class="col-md-3 col-6">
                <div class="kpi-box"><div class="kpi-label">Consommation</div><div class="kpi-value">{{ $consoCalc }}%</div></div>
            </div>
            <div class="col-md-3 col-6">
                <div class="kpi-box">
    ...
    @if(count($chapitres))
    <div class="detail-card">
        <div class="section-title"><i class="bi bi-list-check"></i> Exigences clés SMI</div>
    ...
    <div class="detail-card">
        <div class="section-title">
            <i class="bi bi-journal-check"></i> Suivi des chapitres SMI
        ...
        <div style="margin-top:1.25rem; padding-top:1rem; border-top:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:0.8rem; color:var(--text-secondary); display:flex; align-items:center; gap:0.5rem;">
                <i class="bi bi-check-circle-fill" style="color:var(--success);"></i> Chapitres terminés
            </span>
                ...
                <table class="table-pro">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span style="font-weight:600; color:{{ $pct > 100 ? 'var(--danger)' : 'var(--success)' }};">{{ $pct }}%</span>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <div class="detail-card intervention-section">
        <div class="section-title"><i class="bi bi-folder-fill"></i> Fichiers d'intervention du projet</div>
        <div class="intervention-header">
            <h4><i class="bi bi-file-earmark"></i> Documents et preuves du projet</h4>
...
onclick="viewDocument('{{ $doc->url }}', '{{ $doc->mime_type }}', '{{ addslashes($doc->label ?: $doc->fichier_nom) }}', '{{ addslashes($doc->fichier_nom) }}')"                                title="Voir le document">
                            <i class="bi bi-eye-fill"></i>
                        </button>
                        <button class="intervention-btn"
                                onclick="downloadFile('{{ $doc->url }}', '{{ addslashes($doc->fichier_nom) }}')"
                                title="Télécharger">
                            <i class="bi bi-download"></i>
                        </button>
                        <button class="intervention-btn"
onclick="printDocumentUrl('{{ $doc->url }}', '{{ $doc->mime_type }}', '{{ addslashes($doc->label ?: $doc->fichier_nom) }}')"                                title="Imprimer">
        ...
        <div class="print-document">
            ...
            <div class="print-section">
                ...
                <div class="print-info-grid">
                    <div class="print-info-row"><span class="print-info-label">Code chapitre</span><span class="print-info-value">${chapData.code}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Titre</span><span class="print-info-value">${chapData.titre}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Projet</span><span class="print-info-value">${chapData.projet_nom}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Chef de projet</span><span class="print-info-value">${chapData.chef_nom || 'Non assigné'}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Phase</span><span class="print-info-value">${chapData.phase}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Avancement</span><span class="print-info-value">${chapData.avancement}%</span></div>
                    <div class="print-info-row"><span class="print-info-label">Jours intervention</span><span class="print-info-value">${chapData.jours}</span></div>
                    <div class="print-info-row"><span class="print-info-label">Statut projet</span><span class="print-info-value">${chapData.projet_statut}</span></div>
                </div>
            </div>
            <div class="print-section">
...
function printDocument() {
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                *{margin:0;padding:0;box-sizing:border-box}
                body{font-family:'Inter',sans-serif;padding:2rem;background:white;color:#0f172a;line-height:1.5}
                .print-document{max-width:800px;margin:0 auto}
                .print-header{border-bottom:2px solid #2563eb;margin-bottom:2rem;padding-bottom:1rem;display:flex;justify-content:space-between;align-items:flex-end}
                .print-title{font-size:1.5rem;font-weight:700;color:#2563eb}
                .print-subtitle{font-size:0.8rem;color:#64748b}
                .print-date{font-size:0.8rem;color:#64748b;background:#f1f5f9;padding:0.4rem 1rem;border-radius:100px}
                .print-section{margin-bottom:2rem}
                .print-section-title{font-size:1rem;font-weight:600;color:#0f172a;margin-bottom:1rem;padding-bottom:0.5rem;border-bottom:1px solid #e2e8f0}
                .print-info-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1rem;background:#f8fafc;padding:1.25rem;border-radius:12px}
                .print-info-row{display:flex;border-bottom:1px dashed #e2e8f0;padding:0.6rem 0}
                .print-info-label{width:130px;font-weight:600;color:#64748b;font-size:0.8rem}
                .print-info-value{flex:1;color:#0f172a;font-weight:500;font-size:0.9rem}
                .print-table{width:100%;border-collapse:collapse;margin-top:1rem}
                .print-table th{background:#f1f5f9;color:#64748b;font-size:0.7rem;font-weight:600;text-transform:uppercase;padding:0.6rem;border:1px solid #e2e8f0}
                .print-table td{padding:0.6rem;border:1px solid #e2e8f0;color:#334155;font-size:0.8rem}
                .print-footer{margin-top:2rem;padding-top:1rem;border-top:2px solid #2563eb;text-align:center;color:#64748b;font-size:0.75rem}
                @media print{body{padding:0.25in}.print-section{page-break-inside:avoid}}
            </style>
        </head>
...
function printDocumentUrl(url, mime, title) {
    const isImage = mime && mime.startsWith('image/');
    const isPdf = mime === 'application/pdf';
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closePreuveViewer(); closePrintModal(); }
});
document.getElementById('preuveViewerModal')?.addEventListener('click', e => {
    if (e.target === document.getElementById('preuveViewerModal')) closePreuveViewer();
});
